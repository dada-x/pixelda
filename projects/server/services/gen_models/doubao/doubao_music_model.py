...
from projects.server.services.gen_models.base_service import BaseMusicService
from services.gen_models.utils import save_abc_to_file
...
with open(
    ...
    instruction = f.read()
    MUSIC_SYSTEM_PROMPT = f"You are a helpful assistant. You can generate creative and original music based on the input requirements given to you, and response with ABC format. Use below instruction for ABC format:\n\n{instruction}"


MUSIC_GEN_PROMPT = """
Generate a piece of music notation with ABC format, following requirements: 
duration: {duration} seconds.
genre: {genre}.
tempo: {tempo}.
description: {description}.
Respond only with the ABC notation without any additional explanation.
"""
    ...
    def _execute_request(
        ...
        try:
            ...
            if isinstance(response, ChatCompletion):
                return save_abc_to_file(response.choices[0].message.content)
            else:
